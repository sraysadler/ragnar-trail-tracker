...
**initRaceLegs(preserveActuals=true) must preserve runner, actualFinish, and overrideMiles together**
`initRaceLegs()` is called on every Settings Save to rebuild the leg
data array from config. When called with `preserveActuals=true`, it
must preserve three fields from the existing stored legs: `runner`,
`actualFinish`, and `overrideMiles`. All three must be preserved
together — preserving any subset is a bug.

This was discovered April 18, 2026 when a bug was found where
`runner` was not being preserved (commit bfdea7f). The function
already correctly preserved `actualFinish` and `overrideMiles`, but
unconditionally recalculated `runner` from config defaults, erasing
any manual substitutions when the user saved Settings.

The only paths where `runner` should be re-derived from config are:
fresh load with no prior localStorage data, and explicit Reset. Every
other save path must preserve whatever runner is currently assigned.
...
*Reaffirmed April 18, 2026:* Three independent code reviewers each
suggested generating LEG_DEFINITIONS dynamically (e.g. using
`Array.from({ length: 24 }, ...)` with a LOOPS lookup). The decision
to keep it hardcoded was reviewed and stands. The values are fixed
by the Ragnar Trail race format — they do not vary at runtime, across
race years, or by team. A static constant is the correct representation.
Generating it dynamically would add code complexity with no benefit.
...
and time fields**
Runner name fields and Team/Event name fields use the system keyboard
(text input is appropriate there). All other config fields — start time,
loop distances, avg pace, night adjustment, night hours — open custom
keypads appropriate to their data type.

**Numeric keypads use implied decimal for distance and percentage fields**
Rather than requiring the user to tap a decimal point button, the
numeric keypad for miles and percentage fields uses an implied decimal
one place from the right. Typing `4`, `6` produces `4.6`. This is
faster and eliminates the possibility of entering a non-decimal number
accidentally.
...
**Export button moved from footer to Settings panel**
The Export button was originally in the footer for quick access. It
was moved to the Settings panel (Backup Status section) because export
is a deliberate action, not something needed mid-race at a glance. The
footer space was better used for the Runner Stats button, which is
more frequently useful during the race.

**Backup folder shown as tappable link to change folders**
Once a backup folder was selected, there was no way to change it.
The folder name in the Backup Status section of Settings was made a
tappable blue hyperlink that re-opens the folder picker. This handles
the case where a user wants to change backup locations without
...
**"Race Setup" title on first load, "Settings" thereafter**
When the app is opened for the first time with no saved config, the
Settings panel opens automatically with the title "Race Setup" to
signal that setup is required. Once config is saved, subsequent opens
show "Settings". This distinction helps new users understand they need
to complete setup before the race begins.

**Runner Stats in footer, replacing Export**
A "Runner Stats" button was added to the footer (where Export used to
be) styled in blue to distinguish it from the Settings button. This
gives quick access to per-runner performance data during the race
without navigating to Settings. Export was moved to Settings since
it is a less frequent action.
...
**Loop Miles override via press-and-hold**
To handle edge cases where a runner covers a different distance than
the configured loop distance (e.g. getting lost and running an extra
mile), the Loop Miles field in the Row Detail Modal supports a
per-leg override. A 3-second press-and-hold opens the numeric keypad
to enter a new distance for that leg only. This interaction was chosen
over a tap because it is a deliberate, infrequent action that should
not be triggered accidentally. Only available on completed legs —
you wouldn't know to override until the runner has finished.

**Overridden loop miles shown in green**
When loop miles have been overridden for a leg, the value displays
in green (`#4caf50`) in the Row Detail Modal. This makes it
immediately obvious that a non-default value is in use without
requiring a separate indicator or label.

---

## Runner Stats Modal

**Runner Stats accessible from footer, not from Row Detail Modal**
Runner Stats was added as a footer button rather than being nested
inside the Row Detail Modal. During a race, the team wants to see
all runners at a glance — not just the one on the current leg.
The footer button gives direct access without requiring navigation
through a specific leg first.

**Runner Stats groups by current runner, not default runner**
All legs are grouped and stats calculated by the current `runner`
field, not `defaultRunner`. This correctly handles substitutions —
if Tim takes over Jeff's legs, Tim's stats include all legs assigned
to Tim regardless of who was originally scheduled. Jeff's stats
only include the legs currently assigned to Jeff.

**Runner Stats summary line italic for runners with no completed legs**
A runner with no completed legs shows their summary stats in italic
to signal that the avg pace shown is the config baseline estimate,
not their actual performance. Runners with completed legs show
summary stats in regular weight. No separate label or badge needed —
the italic alone communicates the distinction.

**Pace asterisk for overridden loop miles in Runner Stats**
When a leg's loop miles were overridden, the pace shown in Runner
Stats gets an asterisk (`12:30/mi*`). This signals that the pace
was calculated with a non-standard distance without cluttering
the display with an explanation.

**Team Totals at bottom of Runner Stats**
A Team Totals section shows legs completed, total run time, and
team average pace. These values use only actual data — no estimates.
This gives a quick race-wide summary without having to add up
...
one of its weighted inputs. During planning this was simplified to
use only the runner's own pace history, falling back to the config
baseline when no history exists.
...
**Night pace adjustment applied to pace, not duration**
The adjustment is a percentage applied to the predicted pace for
night legs. This is more accurate than applying it to duration
because it reflects the actual reason for slower night running —
reduced pace due to caution in the dark — rather than just
inflating a time. Applying it to pace also keeps pace history
clean and comparable across day and night legs via normalization.

**Night pace normalization for accurate pace history**
All actual paces are stored as normalized day-equivalent values:
night leg actual paces are divided by (1 + nightAdjustment/100)
before being stored in history. This allows day and night leg
paces to be averaged meaningfully. When predicting a night leg,
the normalized average is re-inflated by the same factor.

**getEffectiveMiles() for per-leg loop distance**
Rather than reading config loop miles directly, all calculations
use a `getEffectiveMiles()` helper that returns the override value
if set, or the config default otherwise. This ensures loop miles
overrides flow through all formulas automatically with no
per-formula conditional logic.

**Runner substitution pace history uses current runner field**
Pace history is built using the `runner` field (current assigned
runner), not `defaultRunner`. This means substituted legs count
toward the substituting runner's history. If Tim takes over Jeff's
legs, Tim's pace history and estimates reflect all legs Tim has run,
including Jeff's original legs.
...
- Distance fields (loop miles): numeric keypad with implied decimal
...
*Document version: 4.0 — Updated April 18, 2026*
*Changes from v3: Added preserveActuals entry under Architecture;
added April 18 reaffirmation note to LEG_DEFINITIONS entry.*
*Companion to: Race Tracker App Plan v7, Race Tracker Wireframes v2,
Race Tracker Do Not Do v1, Race Tracker Data Model v3,
Race Tracker Build Log v4*
