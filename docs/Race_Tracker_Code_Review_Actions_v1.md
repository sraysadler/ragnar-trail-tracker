...
### CR-01 — Runner name injection via inline JS string (HIGH)
...
### CR-02 — CSV restore doesn't handle quoted fields (HIGH — data loss)
...
### CR-03 — Backup files accumulate without limit (MEDIUM — operational)
...
|---|------|----------|----------|
| CR-01 | Runner name HTML/JS injection fix | Bug | Fix before race day |
| CR-02 | CSV restore quoted-field parser | Bug | Fix before race day |
| CR-03 | Backup file accumulation | Bug/Ops | Fix before race day |
| CR-04 | Centralize state object | Architecture | High value |
| CR-05 | Pure `calculateLegData()` | Architecture | High value |
| CR-06 | Unified save pipeline | Architecture | High value |
| CR-07 | Config as native types | Architecture | High value |
| CR-08 | Custom confirm modals | Polish | If time allows |
| CR-09 | AM/PM sanity check | Polish | If time allows |
| CR-10 | Backup write queue | Polish | If time allows |
| CR-11 | CSS variables | Polish | If time allows |
| CR-12 | Strip console.log | Polish | If time allows |
| CR-13 | Timer boundary alignment | Polish | If time allows |
| CR-14 | Long-press discoverability | Polish | If time allows |

---

*Code Review Actions v1.0 — April 18, 2026*
*Ragnar Trail Tracker — github.com/sraysadler/ragnar-trail-tracker*
*Source reviews: GEM (Gemini), CC (Claude Code), CG (Claude)*
*Companion to: Race Tracker Build Log v3, Race Tracker Decisions Log v3*
