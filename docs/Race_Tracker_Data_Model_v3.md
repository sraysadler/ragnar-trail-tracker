...
| Green loop miles | `greenMiles` | Number | User input (numeric keypad, implied decimal) | `4.0` |
| Yellow loop miles | `yellowMiles` | Number | User input (numeric keypad, implied decimal) | `5.0` |
| Red loop miles | `redMiles` | Number | User input (numeric keypad, implied decimal) | `6.0` |
| Estimated team avg pace | `avgPace` | Number (minutes per mile) | User input (pace keypad) | `12.0` |
| Night pace adjustment | `nightAdjustment` | Number (%) | User input (numeric keypad, implied decimal) | `5` |
...
- `avgPace` is treated as a day-equivalent pace — the night adjustment
  is applied on top of it when predicting night legs.
...
| Override miles | `overrideMiles` | Number or `null` | User input via 3-second press-and-hold on Loop Miles in Row Detail Modal. Only available on completed legs. `null` = use config default. |

### Loop Miles Override
When `overrideMiles` is set for a leg, it replaces the config loop
distance for all calculations on that leg only. All other legs continue
to use the config default unless they also have an override.

Use `getEffectiveMiles()` everywhere loop miles are needed per leg:
...
**Never** read `config.greenMiles`, `config.yellowMiles`, or
`config.redMiles` directly when calculating per-leg values. Always
use `getEffectiveMiles(leg, config)`.
...
|-------|-----|------|-------------|
| Effective miles | (via helper) | Number | `getEffectiveMiles(leg, config)` — returns `overrideMiles` if set, else config loop default |
| Start time | `startTime` | String (hh:mm AM/PM) | Leg 1: `raceStartTime`. All others: previous leg's `finishTime` |
| Predicted pace | `predictedPace` | Number (minutes per mile) | See Forecasting Logic below — includes night adjustment if night leg |
| Estimated duration | `estimatedDuration` | Number (minutes) | `predictedPace × getEffectiveMiles()`. Night adjustment already baked into predictedPace. |
...
| Actual pace | `actualPace` | Number (minutes per mile) or `null` | `actualDuration ÷ getEffectiveMiles()`. Null if no actual finish. |
...
own completed leg history. Pace history is grouped by the current
`runner` field — not `defaultRunner`.
...
```javascript
// When storing actual pace in history:
normalizedPace = isNightLeg
  ? actualPace / (1 + nightAdjustment / 100)
  : actualPace;

// When predicting a night leg:
predictedPace = normalizedAvgPace * (1 + nightAdjustment / 100);

// When predicting a day leg:
predictedPace = normalizedAvgPace;
```

Config `avgPace` is treated as a day-equivalent pace (no normalization
needed when used as baseline).

### Predicted Pace Formula

| Prior legs completed | Normalized avg pace | Predicted pace (day leg) | Predicted pace (night leg) | Status |
|---------------------|---------------------|--------------------------|---------------------------|--------|
| 0 | config `avgPace` | `avgPace` | `avgPace × (1 + adj/100)` | `Estimated (Baseline)` |
| 1 | runner's 1 normalized pace | normalized pace | normalized × (1 + adj/100) | `Estimated (Live)` |
| 2+ | equal-weight avg of normalized paces | normalized avg | normalized avg × (1 + adj/100) | `Estimated (Live)` |

### Runner Substitution and Pace History

Pace history is attributed to whoever is currently assigned as `runner`
for each leg. If a runner is substituted into additional legs (e.g. a
runner takes over for an injured teammate), all completed legs assigned
to that runner — regardless of who was originally scheduled — contribute
to their pace history and Runner Stats display.
...
### Runner Stats Derived Data
Computed per runner for the Runner Stats modal:

| Field | Calculation |
|-------|-------------|
| Legs completed | Count of legs where `runner` = this runner AND `actualFinish` is not null |
| Avg pace | Average of actual paces across all completed legs for this runner |
| Team legs completed | Total count of actual finish times entered across all 24 legs |
| Total run time | Sum of all actual durations across all 24 legs |
| Team avg pace | Average of all actual paces across all completed legs |
...
| Leg state (runner, actualFinish, overrideMiles) | `localStorage` | `raceLegs` |
...
Loop Miles exports `getEffectiveMiles()` — override value if set,
...
User overrides loop miles for specific completed legs (optional)
    ↓
calculateLegData() runs 1→24:
  Build runner pace history incrementally (normalized day-equivalent paces)
  Per leg:
    startTime
    → getEffectiveMiles() (override or config default)
    → isNightLeg
    → predictedPace (normalized avg ± night adjustment)
    → estimatedDuration = predictedPace × effectiveMiles
    → finishTime → status
    → actualDuration = actualFinish − startTime (if actual exists)
    → actualPace = actualDuration / effectiveMiles (if actual exists)
    → normalizedPace added to runner history
    ↓
...
- `overrideMiles` stored as a number or `null` — never as a string

---

*Document version: 3.0 — Updated April 17, 2026*
*Companion to: Race Tracker App Plan v7, Race Tracker Wireframes v2,
Race Tracker Do Not Do v1, Race Tracker Decisions Log v2,
