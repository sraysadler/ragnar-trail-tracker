...
## Bug Fixes — April 18, 2026 (post-code-review)

---

## Bug Fix — CR-01: Runner Name HTML/JS Injection ✓

**Identified in code review. Fixed by Claude Code.**

- Added `escapeHtml()` helper for safe name insertion into `innerHTML`
- Replaced all `onclick="selectRunner('${r}')"` inline handlers with
  `data-runner-index` attributes and a single delegated click listener
  on the dropdown container
- Applied `escapeHtml()` to all runner name insertion points: race table
  rows, modal selected-name display, dropdown option labels, and Runner
  Stats section headers
- Verified keypad header (e.g. "Leg 1 – O'Brien") also displays correctly

---

## Bug Fix — CR-02: CSV Restore Naive Comma Split ✓

**Identified in code review. Fixed by Claude Code.**

- Added `parseCSVRow()` — a minimal RFC 4180 parser that correctly handles
  quoted fields and escaped double-quotes
- Replaced all four `row.split(',')` calls in `restoreFromCSV()` with
  `parseCSVRow()`
- `generateCSV()` and `escapeCSV()` untouched — export side was correct

---

## Bug Fix — CR-03: Backup File Accumulation ✓

**Identified in code review. Fixed by Claude Code.**

- `writeBackup()` now always writes to a single canonical file:
  `ragnar_tracker_current.csv` (overwritten each time)
- Manual export via `exportCSV()` → `downloadCSV()` → `buildExportFilename()`
  still produces a unique timestamped filename — unchanged
- No more unbounded file accumulation during a race

---

## Bug Fix — Pace Formatter :60 Rollover ✓

**Discovered during post-code-review testing. Fixed by Claude Code.**

**Bug:** Runner Stats was displaying `11:60/mi` instead of `12:00/mi`
for Kristine's average pace. The average of her three leg paces
(12:35, 11:15, 12:10) produces exactly 12.000 min/mi, but floating
point arithmetic yielded 11.9999...7. The formatter calculated
`Math.round(0.9999 × 60) = 60` seconds, producing `11:60`.

**Fix:** Added a seconds rollover guard to the pace/duration formatting
function. Applied to all MM:SS formatters in the file — pace and
duration displays — not just Runner Stats:
...
## Bug Fix — Runner Name Changes in Settings Not Propagating ✓

**Discovered during post-code-review testing. Fixed by Claude Code.**

**Bug:** If a runner's name was changed in Settings and saved, the new
name did not appear on any legs in the race table. The bfdea7f fix
(preserving `runner` through Settings saves) was preserving all runner
assignments unconditionally — including when the underlying config name
had changed.

**Fix:** On Settings Save, compare each runner slot's new name to the
old name. For any slot where the name changed, update both
`defaultRunner` and `runner` on every leg belonging to that slot to the
new name, overriding any manual substitutions for that slot. For slots
where the name is unchanged, preserve `runner` as before (bfdea7f
behavior intact).

A leg's slot is determined by: `(leg.legNumber - 1) % 8 + 1`

**Behavior:**
- Name unchanged in Settings → manual substitutions preserved
- Name changed in Settings → all legs for that slot update to new name,
  including any that had been manually substituted
...
### Step 26 — UAT Testing
- UAT Checklist v3 ready in /docs — 111 test cases across 16 sections
- Testing to be conducted on Android tablet (primary target device)
- All pre-UAT bug fixes are complete
...
*Build Log version: 5.0 — Updated April 18, 2026*
*Covers: Steps 1–25 complete. Steps 26–27 remaining.*
*April 18 additions: CR-01/CR-02/CR-03 fixed, pace formatter rollover
bug fixed, Settings runner name propagation bug fixed. CR-01/02/03
removed from Known Issues. UAT ready to begin.*
*Companion to: Race Tracker App Plan v7, Race Tracker Data Model v3,*
*Race Tracker Decisions Log v4, Race Tracker Code Review Actions v1*
